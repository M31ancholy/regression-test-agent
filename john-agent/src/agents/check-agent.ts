...
export function createCheckAgent(agentOpt: johnAgentOptions) {
  const customLlmProvider = createOpenAI({
    name: 'third-party',
  ...
  return new ToolLoopAgent({
    ...
    instructions: [
      ...
      '按录制描述选择工具：普通点击用 click，文本输入用 fill，下拉选择用 selectOption，复选/单选用 setChecked。',
      'Enter/Escape 用 pressKey，表单提交用 submitForm，页面滚动用 scrollPage，页内拖拽用 drag。',
      '文件输入用 setFiles，文件拖放用 dropFiles；只传录制描述中的文件名，不得猜测本地路径。',
      '录制值为“[已隐藏]”时不得把该占位文本填入密码框；没有可用测试密码时应将该 todo 标记为 failed。',
