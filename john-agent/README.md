...
Agent tools 覆盖 recorder 产生的普通点击、文本输入、下拉选择、复选框/单选框、Enter/Escape、表单提交、页面滚动、页内拖拽、文件选择和文件拖放。所有页面目标均由模型根据截图提供像素坐标，不需要 DOM selector。

recorder 对文件操作只保存文件名，不复制原文件。要回放文件操作，需要把同名测试文件放入专用目录并配置：

```env
FILE_FIXTURES_ROOT=./fixtures
```

Agent 只能读取该目录下的文件名，绝对路径、路径穿越、越界符号链接和超过 10MB 的文件会被拒绝。

密码录制仅保存 `[已隐藏]` 占位符，不包含可回放的原密码。Agent 不会把占位符填入页面；需要密码步骤时，应另行设计受限的测试凭据注入机制。
